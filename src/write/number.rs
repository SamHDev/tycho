...
use crate::{TychoStatus, Number, NumberIdent, FromResult};
...
pub(crate) fn write_number<W: Write>(writer: &mut W, number: &Number) -> TychoStatus {
    match number {
        ...
        Number::Unsigned8(x) => FromResult::from(writer.write_u8(*x)),
        Number::Signed8(x) =>  FromResult::from(writer.write_i8(*x)),
        Number::Unsigned16(x) => FromResult::from(writer.write_u16::<BE>(*x)),
        Number::Signed16(x) => FromResult::from(writer.write_i16::<BE>(*x)),
        Number::Unsigned32(x) => FromResult::from(writer.write_u32::<BE>(*x)),
        Number::Signed32(x) => FromResult::from(writer.write_i32::<BE>(*x)),
        Number::Unsigned64(x) => FromResult::from(writer.write_u64::<BE>(*x)),
        Number::Signed64(x) => FromResult::from(writer.write_i64::<BE>(*x)),
        Number::Unsigned128(x) => FromResult::from(writer.write_u128::<BE>(*x)),
        Number::Signed128(x) => FromResult::from(writer.write_i128::<BE>(*x)),
        Number::Float32(x) => FromResult::from(writer.write_f32::<BE>(*x)),
        Number::Float64(x) => FromResult::from(writer.write_f64::<BE>(*x)),
    }
}