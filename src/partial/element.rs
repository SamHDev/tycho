    ...
    pub fn read<R: Read + Seek>(reader: &mut PartialReader<R>) -> TychoResult<Self> {
        ...
        match ident {
            ...
            ElementIdent::List => {
                ...
                let pos = reader.pointer.clone();
                reader.jump(&size)?;
                Ok(PartialElement::List(PartialList::new(pos, size, 0, ())))
            },
            ...
            ElementIdent::Array => {
                ...
                if array_type == ValueIdent::Null {
                    return Ok(PartialElement::Array(PartialArray::empty(array_type)))
                }

                let size = read_length(reader)? as u64;
                let pos = reader.pointer.clone();
                reader.jump(&size)?;
                Ok(PartialElement::Array(PartialArray::new(pos, size, 0, array_type)))
            },
            ...
            ElementIdent::Map => {
                ...
                if key_type == ValueIdent::Null {
                    return Ok(PartialElement::Map(PartialMap::empty(key_type)))
                }

                let size = read_length(reader)? as u64;
                let pos = reader.pointer.clone();
                reader.jump(&size)?;
                Ok(PartialElement::Map(PartialMap::new(pos, size, 0, key_type)))
            },
